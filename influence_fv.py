...
class Tracin:
    ...
    def calc_gradients(self, inputs: torch.Tensor, labels: torch.Tensor):
        ...
        with torch.autograd.set_grad_enabled(True):
            ...
            losses = self.criterion(thetas, labels)

            grads = [torch.autograd.grad(inputs=inputs, outputs=loss, retain_graph=True) for loss in losses]
    ...
    def calculate_train_gradients(self):
